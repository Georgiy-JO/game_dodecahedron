...
# Contents
- [Dodecahedron games: Snake](#dodecahedron-games-snake)
- [Contents](#contents)
...
This part of the project consists of two separate components to implement the Snake game: a library responsible for implementing the game logic and a desktop GUI. The game implementation is located in this folder, while GUI realizarion is located at: \\a way to ...

The developed game library also can be connected to the console interface (similar to one, that is used in the separate **The Tetris** project \\link to tetris\\), realixation of which can be found at: \\a way to .... The console interface fully support the game. At the same time project files include [**"The Tetris"** game]() too, due to the fact that the GUE interface fully supports it.  
...
![The_snake_FSM](/ " finite-state machine of The Snake")
