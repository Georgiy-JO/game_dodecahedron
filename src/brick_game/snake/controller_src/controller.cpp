...
void userInput(UserAction_t action, bool hold) {
  gameInterector([&](GameInfo_t&) { gameInput(action, hold); });
}

template <typename Callable>
GameInfo_t gameInterector(Callable&& gameOperation) {
  GameInfo_t game_output{.field = nullptr,
                         .next = nullptr,
                         .score = 0,
                         .high_score = 0,
                         .level = 0,
...
void gameUpdater(GameInfo_t& game) {
  ...
  switch (game_inside->getGameState()) {
    case s21::SnakeGame::st_Exit:
      endGame(game);
      break;
    case s21::SnakeGame::st_Pause:
      game.pause = true;
      break;
    case s21::SnakeGame::st_GameWon:
      game.speed = -1;  // this one will be refactored for github version later
      game.field = nullptr;
      break;
    case s21::SnakeGame::st_GameOver:
      game.field = nullptr;
      break;
    default:
      break;
