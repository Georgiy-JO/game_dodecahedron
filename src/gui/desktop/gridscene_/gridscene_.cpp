...
GridScene_::GridScene_(QMainWindow* parent)
    : scene(new QGraphicsScene(parent)) {}
GridScene_::GridScene_(
    QMainWindow* parent, int grid_width, int grid_height,
    GridConstants::PixelSize_c pixel = GridConstants::ps_Medium)
    : GridScene_(parent) {
...
void GridScene_::gridItemCheck(int x, int y) const {
  if (x >= gr_width || y >= gr_height)
    throw std::out_of_range(
        "GridScene_: you are callin out of range grid item.");
  if (scene == nullptr || grid == nullptr || grid[y][x] == nullptr)
    throw std::out_of_range(
        "GridScene_: grid item you are trying to call is out of range.");
}

int GridScene_::getPixelSize() const { return pixel_size; }
int GridScene_::getWidth() const { return gr_width; }
int GridScene_::getHeight() const { return gr_height; }
const QGraphicsScene* GridScene_::getScene() { return scene; }
...
QColor GridScene_::getItemColorQt(int x, int y) const {
  gridItemCheck(x, y);
  QColor output =
      GridConstants::Colors_c::color(GridConstants::Colors_c::cl_Default);
  if (isItemSquare(x, y))
...
bool GridScene_::isItemBackGround(int x, int y) const {
  bool output = 0;
  if (isItemSquare(x, y) &&
      getItemColorQt(x, y) == GridConstants::Colors_c::color(
                                  GridConstants::Colors_c::cl_BackGround))
    output = dynamic_cast<QGraphicsRectItem*>(grid[y][x])->pen().color() ==
             GridConstants::Colors_c::cl_GridEdge;
  return output;
}
...
void GridScene_::setBackGroundItem(int x, int y) noexcept {  // achtung!!!!
  grid[y][x] = (QGraphicsItem*)(scene->addRect(
      x * pixel_size, y * pixel_size, pixel_size, pixel_size,
      QPen(
          GridConstants::Colors_c::color(GridConstants::Colors_c::cl_GridEdge)),
      QBrush(GridConstants::Colors_c::color(
          GridConstants::Colors_c::cl_BackGround))));
}
void GridScene_::setSquareItem(int x, int y) noexcept {  // achtung!!!!
  grid[y][x] = (QGraphicsItem*)(scene->addRect(
      ...
      QPen(
          GridConstants::Colors_c::color(GridConstants::Colors_c::cl_GridEdge)),
      QBrush(GridConstants::Colors_c::color(
          GridConstants::Colors_c::cl_Default))));
}
void GridScene_::setCircleItem(int x, int y) noexcept {  // achtung!!!!
  grid[y][x] = (QGraphicsItem*)(scene->addEllipse(
      x * pixel_size, y * pixel_size, pixel_size, pixel_size,
      QPen(GridConstants::Colors_c::color(
          GridConstants::Colors_c::cl_BackGround)),
      QBrush(GridConstants::Colors_c::color(
          GridConstants::Colors_c::cl_Default))));
}
void GridScene_::cleanItem(int x, int y) {
  ...
  if (!isItemSquare(x, y)) {
    ...
    setItemColor(
        x, y,
        GridConstants::Colors_c::color(GridConstants::Colors_c::cl_BackGround));
}
...
void GridScene_::setItemColor(int x, int y,
                              QColor color = GridConstants::Colors_c::color(
                                  GridConstants::Colors_c::cl_Default)) {
  gridItemCheck(x, y);
  if (isItemSquare(x, y))
...
void GridScene_::setItem(int x, int y, int item_code) {
  if (item_code != getItemCode(x, y)) {
    switch (item_code) {
      case 0:
        if (!isItemBackGround(x, y)) cleanItem(x, y);
        break;
      case 1:
        if (isItemSquare(x, y))
