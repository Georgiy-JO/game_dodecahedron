...
TEST(SnakeGameTest__snake_, Turn) {
  ...
  int turn_l = s21::snake_::Directions::Left,
      turn_r = s21::snake_::Directions::Right;
...
TEST(SnakeGameTest__field_, Snake_1) {
  ...
  for (int i = 0; i < steps; i++) {
    EXPECT_EQ(field.checkForCrash(), false);
    field.moveSnake();
  }
  EXPECT_EQ(field.checkForCrash(), true);
...
TEST(SnakeGameTest__field_, Basic_2) {
  s21::field_ field;
  int steps = s21::field_::HEIGHT - (s21::field_::HEIGHT / 2 + 2);
  for (int i = 0; i < steps; i++) field.moveSnake();
...
TEST(SnakeGameTest__SnakeGame, Basic_1) {
  ...
  EXPECT_EQ(game.getField(s21::field_::WIDTH / 2, s21::field_::HEIGHT / 2 + 2),
            1);
  EXPECT_EQ(game.getField(s21::field_::WIDTH / 2, s21::field_::HEIGHT / 2 + 1),
            1);
  EXPECT_EQ(game.getField(s21::field_::WIDTH / 2, s21::field_::HEIGHT / 2 + 0),
            1);
  EXPECT_EQ(game.getField(s21::field_::WIDTH / 2, s21::field_::HEIGHT / 2 - 1),
            1);
}
...
TEST(SnakeGameTest__SnakeGame, User_actions) {
  ...
  while (std::chrono::duration_cast<std::chrono::milliseconds>(
             std::chrono::steady_clock::now() - check_timer)
             .count() < 1000) {
  }
  ...
  EXPECT_EQ(game.getField(s21::field_::WIDTH / 2, s21::field_::HEIGHT / 2 + 0),
            1);
  EXPECT_EQ(
      game.getField(s21::field_::WIDTH / 2 - 1, s21::field_::HEIGHT / 2 + 2),
      1);
  ...
  EXPECT_EQ(game.getField(s21::field_::WIDTH / 2, s21::field_::HEIGHT / 2 + 1),
            1);
  EXPECT_EQ(
      game.getField(s21::field_::WIDTH / 2 - 1, s21::field_::HEIGHT / 2 + 3),
      1);
  EXPECT_EQ(
      game.getField(s21::field_::WIDTH / 2 - 1, s21::field_::HEIGHT / 2 + 2),
      1);
  ...
  EXPECT_EQ(game.getField(s21::field_::WIDTH / 2, s21::field_::HEIGHT / 2 + 2),
            1);
  EXPECT_EQ(
      game.getField(s21::field_::WIDTH / 2 - 1, s21::field_::HEIGHT / 2 + 4),
      1);
  EXPECT_EQ(
      game.getField(s21::field_::WIDTH / 2 - 1, s21::field_::HEIGHT / 2 + 3),
      1);
  ...
  game.userActionHandler(s21::SnakeGame::ac_Forvard);
  EXPECT_EQ(
      game.getField(s21::field_::WIDTH / 2 - 1, s21::field_::HEIGHT / 2 + 5),
      1);
  EXPECT_EQ(
      game.getField(s21::field_::WIDTH / 2 - 1, s21::field_::HEIGHT / 2 + 4),
      1);
...
TEST(SnakeControllerTest__InsideFunctions, GameInput_1) {
  ...
  for (int i = 0; i < 10; i++) gameInput(UserAction_t::Up, sw);
...
TEST(SnakeControllerTest__OutsideFunctions, UserInput) {
  ...
  EXPECT_EQ(game->getGameState(), s21::SnakeGame::st_Moving);
  for (int i = 0; i < 10; i++) userInput(UserAction_t::Up, sw);
...
TEST(SnakeControllerTest__InsideFunctions, EndGame) {
  ...
  EXPECT_EQ(game_out.field[s21::field_::HEIGHT / 2 + 2][s21::field_::WIDTH / 2],
            1);
  EXPECT_EQ(game_out.field[s21::field_::HEIGHT / 2 + 1][s21::field_::WIDTH / 2],
            1);
  EXPECT_EQ(game_out.field[s21::field_::HEIGHT / 2 + 0][s21::field_::WIDTH / 2],
            1);
  EXPECT_EQ(game_out.field[s21::field_::HEIGHT / 2 - 1][s21::field_::WIDTH / 2],
            1);
  endGame(game_out);
...
TEST(SnakeControllerTest__InsideFunctions, GameUpdater) {
  ...
  for (int i = 0; i < 10; i++) userInput(UserAction_t::Up, true);
...
TEST(SnakeControllerTest__InsideFunctions, ErrorHandler_2) {
  ...
  std::remove(save_file);
  for (int i = 0; i < 5; i++) userInput(UserAction_t::Up, true);
  auto check_timer = std::chrono::steady_clock::now();
  while (std::chrono::duration_cast<std::chrono::milliseconds>(
             std::chrono::steady_clock::now() - check_timer)
             .count() < 2000) {
...
TEST(SnakeControllerTest__InsideFunctions, GameUpdater_2) {
  s21::SnakeGame* game_inside = s21::gameInstanceKeeper();
  game_inside->gameReStart();
  fieldReallocator();
  ...
  gameUpdater(game);
  int check = 1;
  EXPECT_EQ(game.speed, check);
  EXPECT_NE(game.field, nullptr);
  EXPECT_EQ(game_inside->getGameState(), s21::SnakeGame::st_Moving);
  game_inside->gameOver(1);
  gameUpdater(game);
  game_inside = s21::gameInstanceKeeper();
  check = -1;
  EXPECT_EQ(game_inside->getGameState(), s21::SnakeGame::st_GameWon);
  EXPECT_EQ(game.speed, check);
